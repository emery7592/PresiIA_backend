...
### 1. **Nouvelle fonction `is_greeting_or_intro()`**
Détecte automatiquement :
- Les salutations (bonjour, hello, hey, etc.)
- Les demandes de présentation (qui es-tu, c'est quoi, etc.)
- Les questions de différenciation (pourquoi toi, quelle différence avec ChatGPT)

### 2. **Message de présentation intégré**
Quand une salutation est détectée, le bot répond AUTOMATIQUEMENT avec votre pitch vendeur en 3 points :
- ✅ La vérité avant le consensus
- ✅ Expertise pure relations
- ✅ Stratégies qui marchent vraiment

### 3. **Logique conditionnelle**
```
SI salutation DÉTECTÉE 
  → Message de présentation vendeur
SINON SI thème DÉTECTÉ
  → Navigation vers article pertinent
SINON
  → Réponse normale selon RAG
  """
Centralise la configuration commune au module chat avec RAG.
"""
import os
